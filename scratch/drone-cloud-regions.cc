...
//  Execute ./ns3 run "scratch/UAV_AV.cc --numRegions=1 --dronesPerRegion=5 --avsPerRegion=10 --run=1 --csvFile=uav_av_results_test.csv"
...
int main(int argc, char *argv[])
{
    ...
    uint32_t avsPerRegion = 10;       // Number of autonomous vehicles per region
    uint32_t run = 1;
    double simTime = 60.0;
    std::string csvFile = "experiments/results.csv";

    CommandLine cmd;
    cmd.AddValue("numRegions", "Number of regions", numRegions);
    cmd.AddValue("dronesPerRegion", "UAVs (drones) per region", dronesPerRegion);
    cmd.AddValue("avsPerRegion", "Autonomous vehicles per region", avsPerRegion);
    ...
    bool writeHeader = !std::ifstream(csvFile).good();
